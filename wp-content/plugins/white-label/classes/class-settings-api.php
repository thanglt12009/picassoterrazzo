...
{ // phpcs:ignore
    ...
    public function callback_plugins($args)
    {
        ...
        foreach ($args['options'] as $key => $name) {
            ...
            $html .= '<tr>';
            $html .= '<td class="wl-center">'.sprintf('<input type="checkbox" class="checkbox" id="wpuf-%1$s[%2$s][%3$s]" name="%1$s[%2$s][%3$s]" value="%3$s" %4$s />', $args['section'], $args['id'], $key, checked($checked, $key, false)).'</td>';
            $html .= '<td>'.$name.'</td>';
            $html .= '</tr>';
        }
        $html .= '</tbody>';
